...
public final class ZipExtractionStrategy: PackageExtractionProtocol {
    ...
    public func canHandle(url: URL) -> Bool {
        UTType(filenameExtension: url.pathExtension) == .zip
    }
    
    public func processPackage(sourceURL: URL) throws -> URL {
        try ZFFileManager.shared.clearCache()
        try archiveOperations.extractArchive(file: sourceURL, to: appCacheDirectory, overwrite: true, password: nil)
    ...
    func readAppIconData(from appDirectory: URL, iconName: String, fileName: String?) throws -> Data? {
